...
export function getProductPropertyNames(product) {
  return Object.keys(product).filter(key => typeof product[key] !== 'function');
}

export function getProductPropertyValues(product) {
  return Object.values(product).filter(value => typeof value !== 'function');
}

export function getProductEntries(product) {
  return Object.entries(product).filter(([key, value]) => typeof value !== 'function');
}
